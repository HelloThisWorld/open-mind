...
The main `README.md` references images in this folder. Capture them from the running
...
| `search.png` | Optional future shot: Search box -> an identifier query (e.g. `SocketServer`) | The token-precise results: real code snippets, each with its `file:line`, and the exact-token match indicator. Not referenced by the current README because the current UI emphasizes Glossary and Graphs. |
